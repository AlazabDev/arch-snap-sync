...
export default function Auth() {
      ...
      if (mode === 'register') {
        ...
        if (error) {
          ...
          if (error.message === 'Email not confirmed') {
            throw new Error('لم يتم تأكيد البريد الإلكتروني. تحقق من صندوق الوارد.');
          }
          throw error;
        }
        navigate('/');
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      toast({
        title: 'خطأ',
        description: error.message || 'فشل تسجيل الدخول بجوجل',
        variant: 'destructive',
      });
      setLoading(false);
    }
  ...
  const handleForgotPassword = async () => {
    if (!email) {
      toast({
        title: 'تنبيه',
        description: 'أدخل بريدك الإلكتروني أولاً',
        variant: 'destructive',
      });
      return;
    }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-card rounded-2xl p-8 border border-border shadow-lg">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-background border border-border rounded-xl py-3 font-medium text-foreground hover:bg-muted transition-colors disabled:opacity-50 mb-6"
          >
